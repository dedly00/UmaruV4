...
export const execCommand = async function({api, event, umaru, args, translate}) {
  ...
  if(umaru.data.users[event.senderID] && umaru.data.users[event.senderID].cooldownTime && t - (Date.now() - umaru.data.users[event.senderID].cooldownTime) > 0) {
    ...
    return api.sendMessage((await translate("✨ You have received your rewards for today. Please return in {{h}} hour, {{m}} minutes, and {{s}} seconds.", event, null, true)).replace("{{m}}", minutes).replace("{{s}}", seconds).replace("{{h}}", hours), event.threadID, event.messageID);
  } else {
    let reward = 19011310000;
    umaru.data.users[event.senderID].money += reward;
    umaru.data.users[event.senderID].cooldownTime = Date.now();
    await umaru.save();
    return api.sendMessage((await translate("💵 You were given ${{m}}. Please try again in 12 hours if you want to continue receiving.", event, null, true)).replace("{{m}}", reward), event.threadID, event.messageID);
  }
}